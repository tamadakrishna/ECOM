...
const Cart = () => {
  ...
  return (
   <>
   <div className="laptop:flex laptop:w-screen laptop:h-[calc(100vh_-_60px)] large_screen:hidden ">
    <div className="w-4/5 h-100vh pl-1">
      <div className="w-full h-11 border-b-4 border-gray-400">
        <p className="text-[30px] font-Poppins text-gray-500 ml-1">SHOPPING BAG</p>
      </div>
      <div className="flex w-full h-10 border-b-2 border-gray-400">
        <div className="h-full w-2/4  flex justify-center items-center">
          <span className="text-[#313030] font-Poppins">PRODUCT</span>
          </div>
        <div className="h-full w-1/4  flex justify-center items-center">
          <span className="text-[#313030] font-Poppins">PRICE</span>
        </div>
        <div className="h-full w-1/4  flex justify-center items-center">
          <span className="text-[#313030] font-Poppins">TOTAL</span>
        </div>
      </div>
            ...
            <div key={index} className="flex w-full h-40 border-b-2 border-gray-200  mb-2">
              <div className=" flex h-full w-2/4 ">
                ...
                <div className="h-full w-[calc(100%_-_200px)] flex justify-center items-center">
                  <div>
                  <div><span className="text-[#313030] font-mono">{product?.name}</span></div>
                  <div><span className="text-[#313030] font-mono">Product Id: {(product?.id?.slice(0,10)).toUpperCase()}</span></div>
                  <div><span className="text-[#313030] font-mono">Quantity: {product?.quantity}</span></div>
    ...
    <div className="w-1/5 h-100vh bg-gray-100 pl-2">
      ...
      <p className="text-[20px] text-gray-600 font-bold">SUMMARY</p>
        ...
        <div className="flex w-full h-10 ">
          <div className="w-2/4 h-10 "><span className="text-[#313030] font-Poppins">SUBTOTAL</span></div>
          <div className="w-2/4 h-10 "> &#x20b9; {summary?.subTotal}</div>
        </div>
        <div className="flex w-full h-10 ">
          <div className="w-2/4 h-10"><span className="text-[#313030] font-Poppins">Shipping</span></div>
          <div className="w-2/4 h-10">&#x20b9; {summary?.shipping}</div>
        </div>
        <div className="flex w-full h-10">
          <div className="w-2/4 h-10"><span className="text-[#313030] font-Poppins">Sales Tax</span></div>
          <div className="w-2/4 h-10">&#x20b9; {summary?.salesTax}</div>
        </div>
        <div className="flex w-full h-10">
          <div className="w-2/4 h-10"><span className="text-[#313030] font-Poppins">ESTIMATED TOTAL</span></div>
          <div className="w-2/4 h-10">&#x20b9; {summary?.estimatedTotal}</div>
        </div>
        <div className="w-[100%] h-[60px] flex items-center px-[10px] border-b-[1px] border-gray-300 ">
            <div className="w-[100%] h-[40px] cursor-pointer bg-gray-600 rounded-[5px] flex justify-center items-center"
                 onClick={(e)=>{ 
                  router.push("/shipping")
                  }}>
                <span className="text-[#f6f0f0] cursor-pointer">Proceed to Checkout ({summary?.quantity})</span>
            </div>
        </div>
      </div>
   ...
   <div className="mobile:w-[100vw] mobile:h-[calc(100vh_-_60px)] mobile:overflow-y-scroll small_screen:hidden">
      <div className="mobile:w-[100%] mobile:h-[150px] ">
        <div className="mobile:w-[100%] mobile:h-[35px] ">
          <h1 className="text-[#020617] text-[20px] font-bold ml-[6px]"><span className="text-[20px] font-Poppins  font-normal">Subtotal</span> <sup>&#x20b9;</sup>{summary?.subTotal}</h1>
        </div>
      ...
      </div>
      {
        cart?.map((product,index)=>{
          return (
            <div key={index} className="mobile:w-[100%] mobile:h-[250px] mobile:mb-1 mobile:flex ">
